...
安装和升级 Trident 的众多link:../trident-install/kubernetes-deploy.html["使用啮合式操作员的优势"]之一是自动处理 Trident 和 Kubernetes 对象，而不会中断现有的已挂载卷。通过这种方式，Trident 可以支持零停机时间升级，或link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["_滚动更新_"^]。特别是，Trident 操作程序与 Kubernetes 集群进行通信以：
