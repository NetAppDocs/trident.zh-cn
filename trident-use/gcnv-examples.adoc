...
| `位置` | Asta Trident创建GCNV卷的Google Cloud位置。创建跨区域Kubnetes集群时、在中创建的卷 `location` 可用于在多个Google Cloud区域的节点上计划的工作负载。跨区域流量会产生额外成本。 |  


| `apiKey` | 具有角色的Google Cloud服务帐户的API密钥 `netappcloudvolumes.admin` 。它包括 Google Cloud 服务帐户专用密钥文件的 JSON 格式的内容（逐字复制到后端配置文件）。 `apiKey`必须包括以下键的键值对： `type`、 `project_id`、 `client_email`、、 `client_id` `auth_uri` `token_uri` `auth_provider_x509_cert_url`和 `client_x509_cert_url`。 |  
...
[%collapsible]
====
这是绝对的最低后端配置。通过此配置、Astra Trident会发现在配置位置委派给Google Cloud NetApp卷的所有存储池、并随机将新卷放置在其中一个池中。由于 `nasType` 省略了、因此会 `nfs` 应用默认设置、后端将为NFS卷配置。
