...
.SMB工具
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["准备配置SMB卷"] 如果您正在使用： `ontap-nas` 为 SMB 提供卷。
...
您可以使用以下方式配置 SMB 卷 `ontap-nas` 司机。
