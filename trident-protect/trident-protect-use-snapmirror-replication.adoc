...
NOTE: 使用 CR 时，请在应用 CR 之前手动创建目标命名空间。Trident Protect 仅在使用 CLI 时才会自动创建命名空间。
...
*** *spec.storageClassMapping*：（_Optional_）将源存储类映射到同一 AMR 中各个 PVC 的目标存储类。每个条目都需要一个 `source`和 `destination`存储类名称。设置后， `storageClassMapping`将完全覆盖 `storageClassName`；没有匹配源条目的 PVC 使用集群默认存储类，而不是 `storageClassName`。
+
在 AppMirrorRelationship CR 中使用 `storageClassMapping`时，请注意以下限制：

+
**** `storageClassMapping` 完全覆盖  `storageClassName`。当两个字段都设置时， `storageClassName` 将被忽略。
**** 源存储类在 `storageClassMapping`中没有匹配条目的 PVC 将使用集群默认存储类，而非 `storageClassName`。
**** 每个条目都需要同时提供 `source`和 `destination`值。
**** 更改目标存储类可能会强制删除并重新创建现有目标 PVC。
...
NOTE: 使用 `--storageclass-mapping`将单个源存储类映射到目标存储类。指定一个或多个以逗号分隔的 `source:destination`对（例如， `"class-a:class-b,class-c:class-d"`）。使用 `*:destination-class`作为通配符，将所有未映射的 PVC 映射到单个目标存储类。设置 `--storageclass-mapping`后， `--storage-class`将被忽略。

+
*使用单个目标存储类的示例 (`--storage-class`）：*
