...
NOTE: 默认情况下，Trident 的指标会暴露在目标端口上。 `8001`在 `/metrics`端点。安装Trident时，这些指标*默认启用*。您可以配置通过 HTTPS 在端口上消费Trident指标。 `8444`也一样。
...
您应该定义一个 Prometheus 目标来收集指标并获取有关Trident管理的后端、它创建的卷等的信息。看link:https://prometheus-operator.dev/docs/developer/getting-started/["Prometheus Operator 文档"^]。
...
* Trident AutoSupport 不会收集或传输个人身份信息 (PII) 或个人信息。它附带 https://www.netapp.com/pdf.html?item=/media/14114-enduserlicenseagreementworldwide.pdf["EULA"^]，不适用于 Trident 容器图像本身。您可以了解有关 NetApp 对数据安全和信任承诺的更多信息 https://www.netapp.com/pdf.html?item=/media/14114-enduserlicenseagreementworldwide.pdf["此处"^]。
