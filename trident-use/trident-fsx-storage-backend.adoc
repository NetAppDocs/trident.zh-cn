...
本节中的步骤介绍了如何单独创建后端。有关如何使用 Automatic Backend 功能的信息，请参见 link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["配置 StorageClass 和 PVC"] 部分以了解更多信息。
...
您可以使用  `ontap-nas` 驱动程序配置 SMB 卷。在完成link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["ONTAP SAN 和 NAS 驱动程序集成"]之前，请完成以下步骤：link:worker-node-prep.html#prepare-to-provision-smb-volumes["准备配置SMB卷"]。
