...
NOTE: 要定义通过名称而不是整个命名空间来保护特定 KubeVirt 虚拟机 (VM) 的 VM 范围应用程序，请使用应用程序 CR 中的 `spec.includedVirtualMachines`字段或 CLI 中的 `--virtual-machines`标志。有关详细信息、字段参考和 YAML 示例，请参阅 link:trident-protect-kubevirt-vms.html["使用 Trident Protect 保护 KubeVirt 虚拟机"]。
...
. （_Optional_）如果需要，可以将资源筛选添加到同一 CR 以包括或排除特定资源：
+
** *通用过滤器示例：*
+
...
+ 使用 CLI 创建应用程序时，可以使用 `--annotation`标志指定注释。
...
----
+ * 可以使用 `--resource-filter-include`和 `--resource-filter-exclude`标志根据 `resourceSelectionCriteria`（例如组、种类、版本、标签、名称和命名空间）来包含或排除资源，如以下示例所示：
...
----
+ *要定义仅限 PVC 的应用程序，您还必须在资源筛选器中包含 `PersistentVolume`和 `VolumeSnapshotClass``。快照和备份操作依赖于 `PersistentVolume`（绑定到每个 PVC 的集群范围卷）和 `VolumeSnapshotClass`（快照驱动程序），如果没有它们，则会失败。例如：
