...
您可以通过 StorageClass 传递可选的后端参数。在 StorageClass `parameters` 部分中将所有可选值定义为字符串。有关后端参数的完整列表，请参阅：xref:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples[FSx for NetApp ONTAP 后端配置]。
...
您可以使用  `ontap-nas` 驱动程序配置 SMB 卷。但是，要执行此操作，您必须完成以下步骤：link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["准备配置SMB卷"]。
