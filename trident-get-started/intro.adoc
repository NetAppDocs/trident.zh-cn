...
Netapp Trident 支持在公共云或本地所有流行 NetApp 存储平台上使用和管理存储资源，包括本地 ONTAP 集群（AFF、FAS 和 ASA）、ONTAP Select、Cloud Volumes ONTAP、Element 软件（NetApp HCI、SolidFire）、Azure NetApp Files、Amazon FSx for NetApp ONTAP 和 Google Cloud NetApp Volumes。
...
Trident 支持一系列 Kubernetes 发行版和平台。

支持的平台包括： * 上游 Kubernetes * Red Hat OpenShift * SUSE Harvester 1.7.0 (ONTAP iSCSI)
