...
* 要从存档还原特定 KubeVirt 虚拟机 (VM) 而不还原所有命名空间资源，请使用还原 CR 上的 `includedVirtualMachines`字段。有关支持的 CR、字段引用和 YAML 示例，请参阅 link:trident-protect-kubevirt-vms.html["使用 Trident Protect 保护 KubeVirt 虚拟机"]。
...
** *spec.destinationApplicationName*：（_可选_）已还原应用程序的名称。如果提供，还原的应用程序将使用此名称。如果未提供，还原的应用程序将使用源应用程序名称。
...
[NOTE]
====
* 当您使用 Kopia 作为数据移动器恢复备份时，您可以选择在 CR 中指定注释或使用 CLI 来控制 Kopia 使用的临时存储的行为。请参阅 https://kopia.io/docs/getting-started/["文档"^]有关您可以配置的选项的更多信息。使用 `tridentctl-protect create --help`有关使用Trident Protect CLI 指定注释的更多信息，请参阅命令。
* 使用 CR 恢复到新命名空间时，必须先手动创建目标命名空间，然后再应用 CR。Trident Protect 仅在使用 CLI 时才会自动创建命名空间。
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ .(_Optional_) 如果仅需要选择要还原的应用程序的某些资源，请添加包含或排除标有特定标签的资源的筛选：

+ 注意：Trident Protect 会自动选择某些资源，因为这些资源与您所选资源之间存在关联关系。例如，如果您选择了持久卷声明资源，且该资源具有关联的 pod，则 Trident Protect 也将还原该关联的 pod。

+ * *resourceFilter.resourceSelectionCriteria*：（筛选必需）使用 `Include`或 `Exclude`以包含或排除在 resourceMatchers 中定义的资源。添加以下 resourceMatchers 参数以定义要包含或排除的资源：** *resourceFilter.resourceMatchers*：resourceMatcher 对象数组。如果在此数组中定义多个元素，则它们将作为 OR 操作匹配，并且每个元素（group、kind、version）内的字段将作为 AND 操作匹配。*** *resourceMatchers[].group*：(_Optional_) 要筛选的资源的组。*** *resourceMatchers[].kind*：(_Optional_) 要筛选的资源的类型。*** *resourceMatchers[].version*：(_Optional_) 要筛选的资源的版本。*** *resourceMatchers[].names*：(_Optional_) 要筛选的资源的 Kubernetes metadata.name 字段中的名称。*** *resourceMatchers[].namespaces*：(_Optional_) 要筛选的资源的 Kubernetes metadata.name 字段中的命名空间。*** *resourceMatchers[].labelSelectors*：(_Optional_) 资源的 Kubernetes metadata.name 字段中的标签选择器字符串，如 https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Kubernetes 文档"^]中所定义。例如： `"trident.netapp.io/os=linux"`。
