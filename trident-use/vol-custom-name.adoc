...
* 卷创建、导入和克隆操作。
* 就此而言 `ontap-nas-economy` 驱动程序中，只有 Qtree 卷的名称符合名称模板。
* 就此而言 `ontap-san-economy` 驱动程序中，只有 LUN 名称符合名称模板。
...
* 自定义卷名称仅与ONTAP本地驱动程序兼容。
* 仅对以下情况支持自定义标签： `ontap-san`， `ontap-nas`， 和 `ontap-nas-flexgroup` 司机。
* 自定义卷名称不适用于现有卷。
