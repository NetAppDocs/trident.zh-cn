...
summary: 配置 Trident 以从 Google Cloud NetApp Volumes 进行存储配置。 
---
...
要配置块 (iSCSI) 存储，请参见 xref:trident-use/gcnv-block.html[配置块存储 (iSCSI)]。
...
NOTE: 对于 Google Cloud NetApp Volumes UNIFIED 存储池，Trident 在卷操作期间应用 UNIFIED 特定的命名和验证规则。
