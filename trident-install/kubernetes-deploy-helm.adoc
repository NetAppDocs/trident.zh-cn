...
----
. 使用 `helm install`并指定部署的名称，如下例所示，其中 `100..0`是您要安装的 Trident 版本。
+
[listing]
----
...
| `tridentDebug` | 允许将Trident的日志级别设置为 `debug`。您可以通过与节点健康检查 (NHC) 操作符集成来自动执行强制分离过程。有关信息，请参阅link:../trident-reco/force-detach.html["使用Trident实现有状态应用程序的故障转移自动化"]。 | `false` 
...
| `enableConcurrency`  a| 
启用并发 Trident 控制器操作，以提高吞吐量。有关支持的驱动程序的详细信息，请参见链接：https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation]。
| false 
