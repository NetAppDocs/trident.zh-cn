...
本节中的步骤介绍了如何单独创建后端。有关如何使用 Automatic Backend 功能的信息，请参见 xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[配置 StorageClass 和 PVC] 部分以了解更多信息。
...
您可以使用以下方式配置 SMB 卷： `ontap-nas`司机。在你完成之前 <<ONTAP SAN and NAS driver integration>> 请完成以下步骤： link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["准备配置SMB卷"]。
