...
+
image::../media/aws-eks-01.png[EKS 集群 Add-ons 选项卡]

. 转到*AWS Marketplace附加项*并选择_storage_类别。
+
image::../media/aws-eks-02.png[AWS Marketplace 附加组件存储类别]

. 找到*Next* NetApp Trident并选中Trident插件的复选框，然后单击*Next*。
. 选择所需的附加软件版本。
+
image::../media/aws-eks-03.png[Trident 附加组件版本选择]

. 配置所需的附加组件设置。
+
image::../media/aws-eks-04.png[Trident 附加组件配置设置]
...
+
image::../media/aws-eks-05.png[Trident 附加组件活动状态]
...
----
. 继续设置并配置存储后端。有关信息，请参见link:trident-fsx-storage-backend.html["配置存储后端"]。
