...
[role="lead"]
创建 ONTAP SAN 驱动程序并在 Trident 安装中使用。本节提供后端配置示例以及将后端映射到 StorageClasses 的详细信息。link:https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["ASA r2系统"^] 在其存储层的实现上与其他 ONTAP 系统（ASA、AFF 和 FAS）有所不同。这些差异会影响某些参数的使用，具体如注释所示。link:https://docs.netapp.com/us-en/asa-r2/learn-more/hardware-comparison.html["详细了解 ASA r2 系统与其他 ONTAP 系统之间的差异"^]。在 Trident 后端配置中，无需指定您的系统为 ASA r2。当您选择 `ontap-san`作为 `storageDriverName`时，Trident 会自动检测 ASA r2 或其他 ONTAP 系统。某些后端配置参数不适用于 ASA r2 系统，如下表所示。
...
从 26.06 版本开始，您可以启用 ONTAP 均衡放置，以便 ONTAP 为新卷选择聚合。有关要求和示例，请参阅 link:ontap-balanced-placement.html["使用 ONTAP 后端的均衡放置"]。
...
| `s存储前缀` | 在 SVM 中预置新卷时使用的前缀。以后无法修改。要更新此参数，需要创建新的后端。 | `trident` 
...
NOTE: 当聚合在 SVM 中更新时，它会通过轮询 SVM 自动在 Trident 中更新，而无需重新启动 Trident Controller。当您在 Trident 中配置了特定聚合以配置卷时，如果该聚合被重命名或移出 SVM，则在轮询 SVM 聚合时，后端将在 Trident 中移至失败状态。您必须将聚合更改为 SVM 上存在的聚合，或者将其完全删除，以使后端重新联机。
...
| `formatOptions`  a| 
使用 `formatOptions`为 `mkfs`命令指定命令行参数，该命令在卷格式化时应用。这允许您根据自己的偏好格式化卷。请务必指定与 mkfs 命令选项类似的 formatOptions，不包括设备路径。示例："-E nodiscard"
...
| `加密` | 在新卷上启用 NetApp 卷加密 (NVE)；默认为 `false`。必须在集群上获得许可并启用 NVE 才能使用此选项。如果在后端启用了 NAE，则在 Trident 中配置的任何卷都将启用 NAE。有关详细信息，请参阅：link:../trident-reco/security-reco.html["Trident如何与NVE和NAE配合使用"]。 | “false” *如果指定，则设置为 `true`适用于 ASA r2 系统*。 
...
NOTE: 对于使用 `ontap-san`驱动程序创建的所有卷，Trident 会向 FlexVol 额外增加 10% 的容量以容纳 LUN 元数据。LUN 将按照用户在 PVC 中请求的确切大小进行配置。Trident 向 FlexVol 增加 10%（在 ONTAP 中显示为可用大小）。用户现在可以获得其所请求的可用容量。此更改还可防止 LUN 变为只读，除非可用空间已被完全利用。这不适用于 ontap-san-economy。
...
以下 StorageClass 定义请参阅<<虚拟池后端示例>>。使用 `parameters.selector`字段，每个 StorageClass 指定哪些虚拟池可用于托管卷。卷现在具有所选虚拟池中定义的属性。

* The `protection-gold` StorageClass 映射到 `ontap-san`后端的第一个虚拟池。这是唯一提供黄金级保护的池。
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-not-gold` StorageClass 映射到 `ontap-san`后端的第二个和第三个虚拟池。这些是唯一提供金级以外保护级别的池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* 该 `app-mysqldb` StorageClass 映射到 `ontap-san-economy`后端的第三个虚拟池。这是唯一为 mysqldb 类型应用程序提供存储池配置的池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-silver-creditpoints-20k` StorageClass 映射到 `ontap-san`后端的第二个虚拟池。这是唯一提供银级保护和 20000 积分的池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `creditpoints-5k` StorageClass 映射到 `ontap-san`后端的第三个虚拟池和 `ontap-san-economy`后端的第四个虚拟池。这些是唯一拥有 5000 个信用点的池产品。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `my-test-app-sc` StorageClass 映射到 `ontap-san` 驱动程序中具有 `sanType: nvme` 的 `testAPP` 虚拟池。这是唯一提供 `testApp` 的池。
+
[source, yaml]
----
---
