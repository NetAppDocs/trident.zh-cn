...
[role="lead"]
创建 Autogrow Policies 后，可以查看、更新和删除它们。您还可以监视使用给定策略的卷。
...
IMPORTANT: 更改会影响使用此策略的所有卷。如果可能，请首先在非生产环境中测试更改。
...
Autogrow Policies 使用终结器保护，以防止在卷主动使用时意外删除。
...
----
. 如果卷仍在使用策略，则删除将进入 `Deleting`状态。检查受影响的卷：
+
[source, console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
| 卷使用此策略 | 策略进入 `Deleting`状态。终结器阻止完成，直到删除所有卷。 
...
Autogrow 支持 ONTAP 后端。Autogrow 还支持 NAS 和 SAN 卷的 Google Cloud NetApp Volumes (GCNV)。
...
* *现有卷（棕地部署）：* 即使应用了有效的自动增长策略，自动增长可能也不适用于现有卷。此限制是由于卷发布的持续迁移所致。要确认迁移已完成，请检查 Trident 控制器日志中的 `"Migration completed"`消息。
...
PVC 注释始终优先。如果 PVC 具有 `trident.netapp.io/autogrowPolicy`注释，则无论StorageClass指定什么，Trident 都会使用该值。有关详细信息，请参见 link:automatic-volume-expansion.html#policy-precedence["策略优先级"]。
...
将 PVC 注释设置为 `"none"`。此设置将覆盖该卷的任何 StorageClass 级别策略：
...
Trident 停止扩展该卷。即使使用量继续增加超过  `usedThreshold`，它也不会为该卷创建进一步的调整大小请求。

例如，使用此策略，Trident 在达到 100 GiB 后停止扩展卷：
...
策略使用终结器保护。如果卷仍在使用该策略，则删除将进入 `Deleting`状态并等待您从策略中删除所有卷。
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
删除所有卷后，Trident 释放终结器并删除此策略。



=== Autogrow 支持哪些后端？

Autogrow 支持 ONTAP 后端。它还支持 NAS 和 SAN 卷的 Google Cloud NetApp Volumes (GCNV)。

Autogrow 支持 NFS、iSCSI、FCP 和 NVMe 协议。NVMe 原始块卷需要 ONTAP 9.16.1 或更高版本。

棕色字段部署中的现有卷可能需要在自动扩展生效之前完成卷发布迁移。要验证迁移状态，请检查 Trident 控制器日志：
