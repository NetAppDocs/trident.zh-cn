...
* ONTAP-NAS（仅限 NFS）、ONTAP-SAN（iSCSI、FCP、NVMe）和 Google Cloud NetApp Volumes 驱动程序的并发支持现已全面推出（GA）。
* 添加了对 Amazon FSx for NetApp ONTAP 自动后端配置的支持。当您创建包含所需参数的 StorageClass 时，NetApp Trident 会自动创建相应的后端和 VolumeSnapshotClass（如果需要）。
* 添加了对不同 Microsoft Azure 云的支持，例如 Azure Government 和 Azure China，以及 Azure NetApp Files 后端的自定义云配置。
* 提高 NVMe 卷的控制器可扩展性。
* 添加了对卷自动增长功能的支持。您可以通过定义自定义 NetApp Trident 自动增长策略来控制自动增长行为。
* 添加了对 Google Cloud NetApp Volumes 块存储 (SAN/iSCSI) 的支持。
* 新增对 Google Cloud NetApp Volumes 自动分层的支持。您现在可以通过 NetApp Trident，为 Google Cloud NetApp Volumes 配置 ONTAP FabricPool 自动分层，添加 Kubernetes 原生控制，从而优化存储成本。
* 增加了对 Kubernetes 1.35 的支持。
