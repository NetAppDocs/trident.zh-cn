...
了解 NetApp Trident 和 Trident Protect 的新功能，包括增强功能、修复和弃用。
...
* *Kubernetes*：
+
** 修复了 CSI 发布和卷移动之间的竞争问题，该问题可能导致使用过时的访问信息挂载卷。
** 修复了在节点注册完成之前 NodeGetInfo 可能返回空拓扑标签，导致拓扑感知配置失败的问题。
** 修复了一个罕见的 NVMe 断开连接竞态问题，该问题可能会在另一个 Pod 仍在连接时拆除共享子系统。
** 修复了在并发情况下创建已挂载卷的克隆时 NVMe PVC 克隆发布失败（以及父命名空间映射不正确）的问题。
** 修复了在 ONTAP-SAN-Economy 后端上使用 nameTemplate 的 PVC 快照创建失败的问题。
...
* *Kubernetes*：
+
** 在 ONTAP 后端配置上添加了可选 `storageAPITimeout`（秒），以设置每个后端的 REST 和 ZAPI API 客户端超时。
...
* *Kubernetes*：
+
** 添加了对 ONTAP-NAS-Economy 和 ONTAP-SAN-Economy 驱动程序并发的正式发布 (GA) 支持，将此功能从 Tech Preview 升级，并添加了对 Azure NetApp Files (ANF) 和 SolidFire 驱动程序的 GA 并发支持。
** 增加了对 Kubernetes 1.36 的支持。
** 添加了对 Google Cloud NetApp Volumes (GCNV) NAS 和 SAN 驱动程序的卷自动增长的正式发布 (GA) 支持。
** 添加了 Unified ONTAP 平衡放置功能，以便在配置新卷时进行最佳聚合选择（link:https://github.com/NetApp/trident/issues/64["问题 #64"]）。
** 添加了对 ONTAP-NAS-Flexgroup 驱动程序的 ONTAP AFX 支持。
** 添加了与 NetApp Shift Toolkit 的集成，以便在使用 Red Hat OpenShift Migration Toolkit for Virtualization 时简化虚拟机迁移。
** 为 GCNV SAN 驱动程序添加了 Cloud Identity 支持。
** 添加了对 Trident Volume Publication (TVP) 传播和自动增长工作流的从属卷支持。
** 新增了在启用并发的情况下运行 Trident 时对卷更新和卷组快照的支持。
** 为 GCNV 添加了动态 gRPC 速率限制。
** 添加了控制器就绪性执行探针，并调整了控制器和 sidecar 的默认 Kubernetes API QPS 和 Burst 设置。
** 通过分离连接、加密设置和文件系统格式化步骤，改进了节点操作顺序。
** 将 Trident 容器根文件系统挂载为只读，以提高安全性。
** 在删除克隆卷的源快照时，为 ONTAP 后端中的克隆卷快照添加了尽力而为的快照删除功能。
** 添加了备份克隆 PVC 识别，以防止由 Veeam Kasten、Trilio 和 Cohesity 创建的临时 PVC 卡在 ONTAP 卷恢复队列中。
** 增强的 AWS ARN 处理以支持特殊区域。
** 防止 ONTAP-SAN-ECONOMY 驱动程序在调整大小操作期间 Flexvol 收缩。
...
* *[Tech Preview]*：新增对 ONTAP-NAS-Economy qtree 导入的支持。
* *[Tech Preview]*：增加了对 ASA-R2（SAN 和 NVMe）驱动程序并发性的支持。
* *[Tech Preview]*：为 ONTAP-SAN 和 ONTAP-NAS 驱动程序添加了 GCNV ONTAP 模式 HTTP 传输。
...
* *Kubernetes*：
+
** 修复了并发发布/取消发布、克隆、缓存和后端更新操作的竞争条件和状态处理。
** 修复了 ONTAP-NAS 和 ONTAP-NAS-Economy 驱动程序中从属卷和只读克隆的导出策略竞争条件和并发发布/取消发布处理。
** 修复了当快照策略不同时，在 MetroCluster 故障转移后进行 ONTAP-NAS-Economy FlexVol 发现的问题 (link:https://github.com/NetApp/trident/issues/1082["问题 #1082"])。
** 修复了中断长时间运行的 qtree 删除的 ONTAP-NAS-Economy 卷删除重试（link:https://github.com/NetApp/trident/issues/1121["问题 #1121"]）。
** 修复了 ONTAP-SAN 和 ONTAP-SAN-Economy 导入和调整大小的行为，包括 fsType 验证、卷元数据处理和自动增长模式行为。
** 修复了并发卷创建过程中的 NVMe/TCP 命名空间竞争条件（link:https://github.com/NetApp/trident/issues/1089["问题 #1089"]）。
** 修复了可用路径和门户不匹配时发生的 iSCSI 调整大小失败问题，并改进了重复调整大小的行为。
** 修复了由卷繁忙条件导致的 ONTAP-SAN 卷删除失败问题。
** 修复了 ONTAP REST 错误处理，并改进了 SVM 状态超时行为。
** 修复了 GCNV SAN 分区池和主机组处理、规范化卷标识符转换以及低于最小大小的规范化创建请求。
** 修复了本机 GCNV 容量池选择，以排除 ONTAP 模式池。
** 修复了使用 Trident Automated Workload Failover 加速虚拟机故障转移的虚拟机的 KubeVirt 故障转移。
** 修复了 Operator 管理的安装 RBAC 问题，包括缺少节点 ClusterRoles 以及升级过程中 ClusterRole 和 ClusterRoleBinding 协调不正确的问题。这适用于使用 Trident Operator chart 的 Operator 和基于 Helm 的安装。
** 通过省略默认优先级值并纠正 YAML 模板缩进，修复了 TridentConfigurator 及相关 CRD 上持续存在的 GitOps 差异（link:https://github.com/NetApp/trident/issues/915["问题 #915"]，link:https://github.com/NetApp/trident/issues/1120["问题 #1120"]）。
** 修复了在 Helm 升级期间修补 TridentVolumePublication CRD 的问题（link:https://github.com/NetApp/trident/issues/1151["问题 #1151"]）。
** 修复了 CSI 启动排序问题，现在在节点注册重试之前先启动 CSI gRPC 服务。
** 修复了并发操作期间安全 SMB 克隆配置叠加的问题。
** 修复了升级后 Trident 控制器 Pod 上缺少注释的问题（link:https://github.com/NetApp/trident/issues/1004["问题 #1004"]）。
** 改进了大型 SVM 上的慢速 qtree 配置（link:https://github.com/NetApp/trident/issues/1142["问题 #1142"]）。
** 已修复 ControllerPublish 以使用卷配置文件系统类型，而不是错误的默认文件系统类型。
** 修复了孤立 dm 设备可能导致重复 CSI NodeUnstageVolume 失败的问题。
** 改进了过时节点和多路径清理，在软删除节点清理后删除虚拟多路径残留项和过时的 TridentNode 资源。
...
* 添加了 OpenShift Service CA Operator 集成，以自动化 Trident Protect 准入 Webhook 的 TLS 证书管理。通过在 Helm 值中将  `useOpenshiftServiceCA` 设置为  `true` 来启用。要了解更多信息，请参阅 link:trident-protect/trident-protect-customize-installation.html["定制Trident Protect 安装"^]。
* 新增了对在 OpenShift Virtualization 上备份和恢复 KubeVirt VM 的支持。保护命名空间中的所有 VM 或按名称定位特定 VM，并选择性地从备份或快照中恢复单个 VM。要了解更多信息，请参见 link:trident-protect/trident-protect-kubevirt-vms.html["使用 Trident Protect 保护 KubeVirt 虚拟机"^]。
* 添加了对资源转换的支持，使您可以在还原过程中添加、删除和修改 Kubernetes 资源属性。要了解更多信息，请参阅 link:trident-protect/trident-protect-resource-transformations.html["使用自定义资源还原资源时修改资源"]。
* 新增对  `storageClassMapping` 在 AppMirrorRelationship (AMR) 中的支持，在单个 AMR 定义中实现单个 PVC 的存储类映射。要了解更多信息，请参阅 link:trident-protect/trident-protect-use-snapmirror-replication.html["使用NetApp SnapMirror和Trident Protect 复制应用程序"^]。
...
* 修复了当目标集群上不存在源应用程序命名空间时，跨集群虚拟机还原失败并出现准入 webhook 错误的问题。
* 修复了由于与现有应用程序自定义资源的规范不匹配而导致在应用程序创建步骤失败的选择性虚拟机就地还原问题；现有应用程序现在将被重用。
* 修复了当虚拟机及其 PersistentVolumeClaim 均被删除时，虚拟机就地还原作业挂起在 `Running`状态的问题；缺少 PersistentVolumeClaim 现在将被视为致命错误。
* 已修复编辑现有保护计划错误地触发即时备份或快照运行的问题。
* 已修复快照终结器在应用程序自定义资源被删除后仍尝试获取该资源的问题；现在，当未找到应用程序时，终结器将跳过。
* 修复了在 PersistentVolumeClaim 复制步骤期间使用稀疏副本执行虚拟机就地恢复后，受保护存储容量显著增加的问题。
* 修复了 Trident Protect 在确认删除 ONTAP 中的后备快照之前删除 Kubernetes 快照对象的问题；终结器现在等待 `VolumeSnapshot`删除。
* 修复了当虚拟机冻结操作失败时快照无法继续的问题；冻结和解冻操作现在在失败时仍可继续执行。
* 修复了 KubeVirt 虚拟机应用程序在 AppMirrorRelationship 反向、反向重新同步和重新同步操作期间错误地重新创建 PersistentVolumeClaims 的问题。
...
* *Kubernetes*：
* 修复了 Google Cloud NetApp Volumes (GCNV) SAN 行为，以支持针对块 (iSCSI) 工作负载的 Flex Unified ZONAL 池。现在支持分区和区域配置。
* 通过在 `InternalID`不存在时改进卷创建、验证、命名和回退卷查找操作，修复了 Google Cloud NetApp Volumes (GCNV) NAS UNIFIED 池处理问题。
* 修复了在可用路径和门户不匹配时 iSCSI 设备调整大小操作可能失败的问题。
* 修复了  `blkid` 错误识别 LUKS 设备的文件系统类型、阻止新卷创建的间歇性问题。
* 修复了 LUKS 大规模发布和暂存操作可能会阻止 Trident 控制器的问题。用户现在需要跟踪与卷关联的 LUKS 密码。
* 修复了 iSCSI 多路径分区可能显示为 ghost 设备，从而阻止 CSI `NodeUnstage` 操作成功的问题。
* 修复了并发卷创建过程中的 NVMe/TCP 命名空间竞争条件（问题 #1089）。
* 修复了在同一秒内执行快照时并发克隆快照操作的问题。
* 修复了阻止使用  `dataVolumes` 的 KubeVirt 虚拟机通过 Trident Automated Workload Failover 进行故障转移的问题。
* 修复了使用 Trident 并发核心时删除无效 `TridentMirrorRelationship`自定义资源时发生的错误。
* 修复了  `tridentactionmirrorupdates` Kubernetes CRD YAML 定义中的缩进（问题 #1120）。
* 修复了 REST qtree 异步删除 API 调用（Issue #1121）。
* 已修复 `ControllerPublish`使用卷配置文件系统类型而不是错误的默认文件系统类型的问题。
* 已修复控制器启动日志记录以正确报告并发驱动程序状态。
* 修复了并发 Trident 中暂停后端的处理。
* 增强的 AWS ARN 处理以支持特殊区域。
...
* Go 运行时：link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* 其他依赖项：link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]
...
* *Kubernetes*：
+
** 添加了对 ONTAP-NAS（仅限 NFS）、ONTAP-SAN（iSCSI、FCP、NVMe）和 Google Cloud NetApp Volumes (GCNV) 驱动程序并发性的一般可用性 (GA) 支持，使此功能从 Tech Preview 升级。有关详细信息，请参见link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["控制器可扩展性文档"]。
** 添加了对 Trident 基于自定义、用户定义的 Trident AutoGrow 策略的卷自动增长的支持。有关详细信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["自动卷扩展文档"]。
** 增强 Trident 节点并发性，提高 NVMe 卷节点操作的可扩展性。有关详细信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["NVMe 卷文档"]。
** 添加了对通过 PVC 注释(`tieringPolicy` `tieringMinimumCoolingDays`进行 GCNV NAS 卷自动分层的支持，包括池选择和克隆继承。有关详细信息，请参见link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["为 Google Cloud NetApp Volumes 文档配置自动分层"]。
** 添加了对使用  `google-cloud-netapp-volumes-san` 驱动程序的 GCNV SAN 块 (iSCSI) 卷的支持，包括调配、针对 LUN 访问的每节点主机组映射以及卷克隆操作。有关详细信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Google Cloud NetApp Volumes 块配置文档"]。
** 添加了对 Amazon FSx for NetApp ONTAP 的自动后端配置支持。当您创建包含所需参数的 StorageClass 时，Trident 会自动创建相应的后端和 VolumeSnapshotClass（如果需要）。有关更多信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAP 后端配置文档"]。
** 添加了对不同 Microsoft Azure 云的支持，例如 Azure Government 和 Azure China，以及 Azure NetApp Files 后端的自定义云配置。有关详细信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Azure NetApp Files 后端配置文档"]。
** 增加了对 Kubernetes 1.35 的支持。有关详细信息，请参见 link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["需求文档"]。
...
* *[Tech Preview]*：增加了对 ONTAP-NAS-Economy 和 ONTAP-SAN-Economy 驱动程序的并发支持。
...
* *Kubernetes*：
+
** 修复了在 ONTAP-NAS、ONTAP-NAS-Economy 和 Google Cloud NetApp Volumes 驱动程序中取消发布只读克隆从源卷删除导出策略规则的问题link:https://github.com/NetApp/trident/issues/1086["问题 #1086"]。
** 将 kubectl 映像切换为基于 Alpine 的轻量级变体，以防止 Bitnami 公开映像弃用后的拉取失败 https://github.com/NetApp/trident/issues/1080["问题 #1080"]。
** 修复了 Trident 升级期间保留现有部署的注释link:https://github.com/NetApp/trident/issues/1004["问题 #1004"]的问题。
** 如果两个存储类都指向同一后端，则允许跨不同存储类进行克隆link:https://github.com/NetApp/trident/issues/1104["问题 #1104"]。
** 修复了具有网络延迟的云环境中超时导致的节点准备故障。增加了基于云的安装的超时值。
** 修复了创建 LUN 时导致文件系统类型属性在进程进入重试状态时保持未设置的问题。
** 修复了 REST API 卷查找以忽略卷状态，从而防止在卷查询期间出现错误否定。
** 大规模使用时，提高了 ontap-nas-economy 驱动程序的 Trident 控制器效率。
** 在 ontap-san-economy 驱动程序中设置 LUN 导入期间的 internalID。
** 增加 Azure Resource Graph 查询限制以处理更多子网。
** 改进了 CSI 和 ONTAP 克隆拆分超时，以避免某些备份应用程序出现竞争情况link:https://github.com/NetApp/trident/issues/1098["问题 #1098"]link:https://github.com/NetApp/trident/issues/1100["问题 #1100"]。
** 修复了 LUKS 错误消息的抑制 link:https://github.com/NetApp/trident/issues/1069["问题 #1069"]。
** 修复了对 iSCSI 和 NVMe 协议的过时 LUKS 映射器的处理。增强的清理逻辑可防止来自孤立设备映射器的装载失败。


* 已修复 RWX NVMe 卷的规模限制。
* 已更新 OpenTelemetry-Go 软件包以修复 link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"]。
...
* Trident Protect 现在会自动禁用保护计划，并在就地还原之前取消正在进行的操作，并在还原完成后重新启用它们。要了解更多信息，请参阅 link:trident-protect/trident-protect-restore-apps.html["使用Trident Protect 恢复应用程序"^]。
* 将 `runImmediately` 字段添加到计划 CR 和 `--run-immediately` CLI 标志，以便在计划创建时触发即时备份或快照。要了解更多信息，请参阅link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["创建数据保护计划"^]。
* 添加了使用还原 CR 中的 `destinationApplicationName`字段或 `--destination-app-name` CLI 标志为还原的应用程序指定自定义名称的支持。要了解更多信息，请参阅link:trident-protect/trident-protect-restore-apps.html["使用Trident Protect 恢复应用程序"^]。
...
* 修复了在所需服务帐户可用之前创建 pod 导致的还原失败。
* 已修复在应用程序还原期间跳过  `Roles` 和  `RoleBindings` 的问题。
* 修复了尽管已正确配置，但未在  `tridentctl-protect get appvaultcontent` 输出中显示原始集群名称的问题。
* 修复了由于缺少  `pipefail` 处理而被忽略的 Kopia 恢复错误。
* 修复了由排除持久卷的资源筛选器导致的快照和备份失败。
* 修复了跨命名空间使用相同命名 PVC 的多命名空间应用程序中不正确的 PVC 还原，这可能会导致数据丢失。
...
** 为ONTAP-NAS 和ONTAP-NAS-Economy（NAS 驱动程序中不包括 SMB）以及ONTAP-SAN 和ONTAP-SAN-Economy 驱动程序添加了强制卷分离的自动工作负载故障转移支持。看link:https://docs.netapp.com/us-en/trident/trident-reco/force-detach.html["使用Trident实现有状态应用程序的故障转移自动化"^]。
...
** 新增了一个选项，可以使用 HTTPS 而不是 HTTP 来提供 Prometheus 指标。看link:https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html["监控Trident"^]。
** 添加了一个选项 `--no-rename`导入卷时，保留原始名称，但让Trident管理卷的生命周期。看link:https://docs.netapp.com/us-en/trident/trident-use/vol-import.html["导入卷"^]。
...
+
看link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["支持的备份和计划注释"^]。



* 在计划 CR 中添加了一个注释，用于配置 PVC 绑定超时，该超时将由备份 CR 使用： `protect.trident.netapp.io/pvc-bind-timeout-sec` 。看link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["支持的备份和计划注释"^]。
...
IMPORTANT: 对于使用 SolidFire 的客户，由于取消发布卷时出现已知问题，请勿升级到 25.06.1。25.06.2 将很快发布以解决此问题。
...
** 修复了未设置 `nfsMountOptions`时 GCNV 卷默认为 NFSv3 的问题；现在支持 NFSv3 和 NFSv4 协议。如果未提供 `nfsMountOptions`，则使用主机的默认 NFS 版本（NFSv3 或 NFSv4）。
